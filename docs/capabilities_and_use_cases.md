# Metrifid: Capabilities, Use Cases, and Current Boundaries
...
- future ideas that are **not** part of the current product.
...
Metrifid is a local MuJoCo assurance library and command-line tool for answering three different questions:

1. **Did two MuJoCo source trees compile into the exact same complete compiled model?**  
   Use `metrifid certify`.

2. **Did a candidate model behave differently from a baseline on one exact declared workload?**  
   Use `metrifid compare`.

3. **Which declared larger MuJoCo timestep remains within the tolerances of one exact workload?**  
   Use `metrifid audit-timestep`.
...
Metrifid does not treat source-code similarity, successful compilation, visual inspection, or one arbitrary simulation run as sufficient proof. It binds each result to exact source bytes, runtime identity, declared workload inputs, deterministic outputs, and machine-readable receipts.

The strongest current product idea is:

> **Use `certify` first. When the complete compiled artifacts differ, use `compare` to determine whether the difference matters on the workload you care about. Use `audit-timestep` when the decision is how far the simulation timestep can be increased without exceeding your declared fidelity limits.**

---

## Current product state

The release described here is:

```text
Distribution:       metrifid
Python package:     metrifid
CLI command:        metrifid
Version:            0.2.1
License:            Apache-2.0
Copyright:          2026 Volodymyr Barylyak
Contributions:      DCO 1.1 sign-off
Development status: alpha
```

The product logic, documentation structure, source-closure protections, output-ownership protections, command interfaces, and receipt formats are implemented.

Release evidence is generated by CI rather than maintained by hand here. Each release runs the complete installed-wheel suite, a seed-17 randomized run, the 29-check native MJB characterization, the bundled first-use demonstration, and the SDK examples across the blocking compatibility matrix, and retains those results as workflow artifacts.

```text
Controlled SDK use on the tested runtime: technically ready
Shareable source repository: ready
Public PyPI publication: not yet completed
```

This document describes product capabilities and current release boundaries. It does not assert that a PyPI release has already been published.
...
MuJoCo Python package 3.10.0 or binding-only 3.10.0.postN
MuJoCo native engine exactly 3.10.0 / mj_version 3010000
...
Validation has passed on Darwin x86_64 with CPython 3.12.13 and MuJoCo 3.10.0. The repository CI defines additional Python and platform lanes; a tuple should be described as validated only after its exact lane passes. MJB digests are runtime-bound and are not expected to match across platforms or Python environments.
...
# 10. Future ideas that are not current capabilities

The following are **not currently implemented product features**:

- dynamics-only compiled projections;
- automatic model inspection and proposed alias maps;
- sensor-first comparison metrics;
- actuator-force comparison as a standard metric;
- workload detection-floor certification;
- time-to-divergence growth-rate analysis;
- solver/contact-capacity qualification;
- CPU MuJoCo versus MJX/MJWarp parity;
- cross-MuJoCo-version drift commands;
- in-toto attestations;
- Sigstore signing;
- OCI publication;
- Model BOM export;
- team baseline and workload registries;
- hosted history and policy controls;
- enterprise on-premises evidence management.


---

# 11. Best current value proposition
...
# 12. Recommended command selection
...
- public PyPI publication still requires the owner-bound repository metadata, blocking public CI matrix, exact distribution upload, anonymous installation, and public-example verification.
